...
window.onload = async function () {
    ...
    let dodatanReFirstLastName = /^[A-Z]([\w\dŽĐŠĆČćđčžšшђжћчЂШЖЋЧ]{2,29}[\d!@#\$%\^&\*\._]+)|([\d!@#\$%\^&\*\._]+[\w\dŽĐŠĆČćđčžšшђжћчЂШЖЋЧ]{2,29})$/;
    ...
    submitButton.addEventListener('click', async () => {
        ...
        if (!greska) {
            ...
            if (msg == "confirm") {
                ...
                $('#success-modal .modal-body').html(`A confirmation mail has been sent to "${signupEmail.value}"`);
    ...
    // else if (url == "/MaxShoes/products.php") {
        ...
        const listaDodatnihFiltra = document.querySelectorAll('#ostali-filtri input[type="checkbox"]');
        ...
        let kategorijeTekst = Array.from(listaKategorija).map(kategorija => kategorija.textContent)
        let brendoviTekst = Array.from(listaBrendova).map(kategorija => kategorija.textContent)
        let sortirajTekst = Array.from(listaSortiranja).map(kategorija => kategorija.textContent)
        ...
        function UkloniRoditelja(dugme) {
            ...
            if (dugme.parentElement.textContent.split(': ')[0] == "Keyword") $('#text-input').val("");
        ...
        async function glavniFiltar() {
            ...
            data.getShoes = true;
            let filtriranePatike = konvertujPatike(await ajaxCall("filter.php", "post", data));
        ...
        async function stampajPatike(shoeList) {
            ...
            let idDodatihPatika = Array.from(document.querySelectorAll('.shoe-data')).map(el => parseInt(el.textContent));
            ...
            for (let shoe of shoeList) {
                ...
                <img src="${shoe.slika_src}" alt="${shoe.slika_alt}" class="img-fluid">
                <strong class="d-block">${shoe.brend_naziv} ${shoe.patika_model}</strong>
                <i class="d-inline-block ${bgColor} w-75 rounded">${shoe.kategorija_naziv}</i>
                <div id="shoe-price">
                ${shoe.popust ? 
                    "<p class='text-decoration-line-through d-inline mx-2 text-danger fst-italic'>$" + shoe.bazna_cena + 
                    "</p>" + "<p class='d-inline text-success fw-bold'>$" + shoe.snizena_cena + 
                    "<strong class='d-inline text-black fst-italic'>(" + Math.round((1 - shoe.popust) * 100) + "% off!)</strong>" :
                    "<p>$" + shoe.bazna_cena + "</p>"
                }
                </div>
                ${shoe.cena_postarine ? "<i id='text-shipping' class='bg-dark d-inline-block text-white p-3 rounded-pill'>Shipping: $" + shoe.cena_postarine + "</i>" : 
                ...
                <svg data-id="${shoe.patika_id}" xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-cart3 korpa ${idDodatihPatika.includes(shoe.patika_id) ? 'crvena' : 'plava'}" viewBox="0 0 16 16" data-bs-title="Remove from cart">
    ...
    //* * * * * * * * * * * * * * * * * * * * * * * * * * CART STRANICA * * * * * * * * * * * * * * * * * * * * * * * * * * *//
        ...
        if (idDodatihPatika[0]) {
            stavkeUKorpi = konvertujPatike(await ajaxCall("filter.php", "post", data));
        }
        ...
        function stampajKorpu(stavkeUKorpi) {
            ...
            for (let stavka of stavkeUKorpi) {
                total += stavka.cena_postarine ? stavka.snizena_cena + stavka.cena_postarine : stavka.snizena_cena;
                let bgColor;
                if (stavka.kategorija_naziv == 'Men') bgColor = 'dark-blue-bg text-white';
                ...
                <i class="fa-solid fa-trash obrisi-stavku" data-id="${stavka.patika_id}"></i>

                <div class="col-md-5 d-flex flex-column border-bottom p-2">
                <hgroup>
                <h2 class="text-decoration-underline">${stavka.brend_naziv} ${stavka.patika_model}</h2>
                <i class="${bgColor} p-2 my-1 d-inline-block rounded">${stavka.kategorija_naziv}</i>
                </hgroup>
                </div>
                <div class="col-md-3 d-flex flex-column border-bottom p-2">
                <h2 class="text-decoration-underline">Price:</h2>
                ${stavka.popust ? 
                    "<hgroup><h5 class='text-decoration-line-through d-inline text-danger fst-italic'>$" + stavka.bazna_cena + 
                    "</h5>" + "<h5 class='d-inline text-success fw-bold mx-2'>$" + stavka.snizena_cena + "</h5></hgroup>":
                    "<h5>$" + stavka.bazna_cena + "</h5>"
                }
                <p>+$${stavka.cena_postarine ? stavka.cena_postarine : '0.00'} shipping</p>
                </div>
                <div class="col-md-4 d-flex border-bottom">
                <img class="img-fluid p-2" src="${stavka.slika_src}" alt="${stavka.slika_alt}">
                </div>
                </div>
                `
            }
        ...
        function dodajEventKantama(stavkeUKorpi) {
            document.querySelectorAll('.obrisi-stavku').forEach(dugme => {
                dugme.addEventListener('click', async function () {
                    let shoeToRemove = stavkeUKorpi.find(stavka => stavka.patika_id == dugme.dataset.id);
                    stavkeUKorpi = stavkeUKorpi.filter(stavka => stavka.patika_id != shoeToRemove.patika_id);
...
function konvertujPatike(shoes) {
    let convertedShoes = shoes.map(shoe => {
        return {
            ...
            cena_postarine: shoe.cena_postarine ? parseInt(shoe.cena_postarine) : null,
    ...
    $(document).ready(function () {
        ...
        $('.korpa').click(async function () {
            ...
            if ($(this).hasClass('plava')) {
                let shoeToAdd = shoeList.find(shoe => shoe.patika_id == this.dataset.id)
                await ajaxCall("cartmanager.php", "post", {
                    shoeToAdd
                });
                brojUKorpi++;
                ...
                let shoeToRemove = shoeList.find(shoe => shoe.patika_id == this.dataset.id);
                await ajaxCall("cartmanager.php", "post", {
                    shoeToRemove
                });
                brojUKorpi--;
